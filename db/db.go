...
func createTables() {
	...
	createAccounts := "CREATE TABLE IF NOT EXISTS `accounts` (`id` INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, `user_id` TINYBLOB NOT NULL, `name` VARCHAR(255) NOT NULL, `email` VARCHAR(255) NOT NULL, `password` BINARY(60) NOT NULL, `permission_level` TINYINT NOT NULL, `school_id` VARCHAR(255), `timetable_list` LONGBLOB, `timetable_is_public` BOOL,`grade` TINYINT, `class` TINYINT, `number` TINYINT, `checklist_id` INT(11), `friends` LONGBLOB) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;"
